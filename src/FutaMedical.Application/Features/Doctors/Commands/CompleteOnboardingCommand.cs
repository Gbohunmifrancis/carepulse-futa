...
public record CompleteOnboardingCommand : IRequest<ApiResponse<object>>
{
...
public class CompleteOnboardingCommandHandler : IRequestHandler<CompleteOnboardingCommand, ApiResponse<object>>
{
    ...
    public async Task<ApiResponse<object>> Handle(CompleteOnboardingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (doctor.ApplicationStatus != null)
            return ApiResponse<object>.BadRequest("Onboarding application has already been submitted");

        // Check if license number already exists
        if (await _context.Doctors.AnyAsync(d => d.LicenseNumber == request.LicenseNumber && d.Id != doctor.Id, cancellationToken))
            return ApiResponse<object>.BadRequest("License number already exists");
        ...
        if (department == null)
            return ApiResponse<object>.BadRequest("Department not found or inactive");
        ...
        return ApiResponse<object>.Ok(new { }, "Onboarding completed successfully. Your application is under review.");
    }
}
